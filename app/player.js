...
/* ====== A PARTIR DAQUI: função de inserção/carregamento corrigida ====== */
...
function hasAudioInMime(mime){
  // heurística: progressive MP4 geralmente traz mp4a no codecs
  return /mp4a/i.test(mime || "");
}
...
playerxhttpr.onload = function() {
  if (playerxhttpr.status === 200) {
      const data = JSON.parse(playerxhttpr.response);
          ...
          // ===== STORYBOARD (usa a mesma URL escolhida, só para pré-visualização simples) =====
          // SBVideo será definido após escolhermos a melhor fonte

          // ===== COLETA E ORDENA OS FORMATOS =====
          const allFormats = Array.isArray(data.formats) ? data.formats.slice() : [];

          // Preferência 1: MP4 com áudio embutido (progressive), ordenado por resolução desc.
          const mp4WithAudio = allFormats
            .filter(f => f && f.mimeType && f.url && f.qualityLabel && f.mimeType.includes("video/mp4") && hasAudioInMime(f.mimeType))
            .sort((a,b) => parseQualityLabel(b.qualityLabel) - parseQualityLabel(a.qualityLabel));

          // Preferência 2 (fallback): MP4 qualquer (pode ser só vídeo). Ainda assim ordenado por resolução
          const mp4Any = (mp4WithAudio.length ? [] : allFormats
            .filter(f => f && f.mimeType && f.url && f.qualityLabel && f.mimeType.includes("video/mp4"))
            .sort((a,b) => parseQualityLabel(b.qualityLabel) - parseQualityLabel(a.qualityLabel)));

          // Preferência 3 (fallback final): qualquer vídeo com qualityLabel
          const anyVideo = (mp4WithAudio.length || mp4Any.length ? [] : allFormats
            .filter(f => f && f.url && f.qualityLabel && /video\//.test(f.mimeType || ""))
            .sort((a,b) => parseQualityLabel(b.qualityLabel) - parseQualityLabel(a.qualityLabel)));

          const sortedFormats = mp4WithAudio.length ? mp4WithAudio : (mp4Any.length ? mp4Any : anyVideo);

          if (!sortedFormats.length) {
            // nada viável -> erro amigável
            videoPlayer.classList.add("player-has-error");
            playerError.textContent = `No playable formats were found for this video

Sorry about that...`;
            console.error("No playable formats found in response.");
            return;
          }

          // ===== CRIA AS <source> (em ordem da melhor qualidade para a pior) =====
          let bestSourceURL = null;
          sortedFormats.forEach(function(item, index) {
            const vidSource = document.createElement("source");
            vidSource.src = item.url;
            vidSource.type = item.mimeType || "video/mp4";
            vidSource.setAttribute("label", item.qualityLabel || "");
            vidSource.setAttribute("selected", index === 0 ? "true" : "false");
            video.appendChild(vidSource);
            if (index === 0) bestSourceURL = item.url;
          });

          // Define a melhor fonte inicial
          if (bestSourceURL) {
            video.src = bestSourceURL;
          }

          // Define storyboard para a mesma fonte (simples)
          if (bestSourceURL) {
            SBVideo.src = bestSourceURL;
          }

          // Legendas (se presentes)
          if (data.captions && data.captions.captionTracks) {
